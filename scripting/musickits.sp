...
public int Menu_MusicKits(Menu menu, MenuAction action, int client, int param)
{
	if (action == MenuAction_Select)
	{
		...
		if (g_cMessage.BoolValue)
		{
			...
			eItems_GetMusicKitDisplayNameByDefIndex(g_iMusicKit[client], sDisplay, sizeof(sDisplay));
...
void ShowMusicKitsMenu(int client)
{
	...
	for (int i = 0; i <= eItems_GetMusicKitsCount(); i++)
	{
		...
		eItems_GetMusicKitDisplayNameByDefIndex(defIndex, sDisplayName, sizeof(sDisplayName));
